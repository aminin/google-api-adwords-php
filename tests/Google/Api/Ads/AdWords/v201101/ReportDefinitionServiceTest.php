...
require_once 'Google/Api/Ads/AdWords/v201101/ReportDefinitionService.php';
...
class ReportDefinitionServiceTest extends AdsTestCase {
  ...
  protected function setUp() {
    ...
    $mccUser = $this->sharedFixture['mccUser'];
    $this->mccService = $mccUser->GetService('ReportDefinitionService');
  ...
  /**
   * Test adding a cross-client report definition.
   * @covers ReportDetinitionService::mutate
   */
  public function testAddCrossClient() {
    $selector = new Selector();
    ...
    $reportDefinition = new ReportDefinition();
    $reportDefinition->selector = $selector;
    $reportDefinition->reportName = 'Test cross-client report #' . time();
    ...
    $operation = new ReportDefinitionOperation($reportDefinition, 'ADD');
    $results = $this->mccService->mutate(array($operation));
  ...
  /**
   * Test getting a report definition.
   * @covers ReportDetinitionService::get
   */
  public function testGet() {
    $selector = new ReportDefinitionSelector();
    $selector->definitionIds[] = $this->reportDefinitionId;
  ...
  public function testGetReportFields($reportType) {
    ...
    foreach ($fields as $field) {
      ...
      $this->assertNotNull($field->displayFieldName);
      $this->assertNotNull($field->xmlAttributeName);
      ...
      if (isset($this->enumValues)) {
        $this->assertNotNull($field->enumValuePairs);
        foreach ($field->enumValuePairs as $pair) {
          $this->assertNotNull($pair->enumValue);
          $this->assertTrue(array_search($pair->enumValue) !== FALSE);
          $this->assertNotNull($pair->enumDisplayValue);
        }
      }
    }
  }
   ...
   * @return array an array of report types and fields (as an array)
   */
  public function reportProvider() {
    ...
    $data[] = array('ACCOUNT_PERFORMANCE_REPORT', array('Date', 'Clicks'));
    ...
    $data[] = array('DEMOGRAPHIC_PERFORMANCE_REPORT',
        array('Gender', 'Clicks'));
    $data[] = array('GEO_PERFORMANCE_REPORT',
        array('CountryCriteriaId', 'Clicks'));
    $data[] = array('AD_EXTENSIONS_PERFORMANCE_REPORT',
        array('AdExtensionId', 'Clicks'));
    $data[] = array('DESTINATION_URL_REPORT',
        array('EffectiveDestinationUrl', 'Clicks'));
    $data[] = array('CREATIVE_CONVERSION_REPORT',
        array('ConversionTypeId', 'Conversions'));
